---
name: meuframework-router
description: Template de roteador entre uma skill de conhecimento e o RAG híbrido knowledge-rag. O operador docops substitui o slug sem copiar e colar e decide: conceitual → skill; factual/literal → RAG com citação; ambígua/alto risco → ambos e declarar divergência.
metadata:
  type: router
  kind: orchestration
...
# meuframework-router — quando usar skill, quando usar RAG

## Estado do sistema

- Skill estática: o pacote `skill/SKILL.md` gerado por `docops` e, quando disponível, enriquecido pela Agent Skill `book-to-skill` do harness.
- RAG MCP: servidor `knowledge-rag` (tools: `search_knowledge`, `get_document`, `search_similar`, `reindex_documents`, ...). O pacote registra o corpus em `rag/documents/`.
...
- Quando algo vem da skill estática, indicar o arquivo da skill (ex.: `meuframework/SKILL.md`, `meuframework/chapters/03-retries.md`).
...
- Pergunta sobre <framework-alvo>: sempre passar por este roteador antes de responder direto.
